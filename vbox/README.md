...
```bash
packer init vbox/vbox.auto.pkrvars.hcl
packer build vbox/vbox.auto.pkrvars.hcl
```
...
- http_directory: Path to a directory to serve using an HTTP server.
    - The files in this directory will be available over HTTP that will be requestable from the virtual machine.
    - By default empty string, no HTTP server will be started.
    - The address and port of the HTTP server will be available in the environment variables `{{ .HTTPIP }}` and `{{ .HTTPPort }}`.
